...
test('should use `naming.typeOf()` for block', t => {
    ...
    t.true(t.context.stub.calledWith({ block: 'block' }));
});
...
test('should use `naming.typeOf()` for elem', t => {
    ...
    t.true(t.context.stub.calledWith({ block: 'block', elem: 'elem' }));
});
...
test('should use `naming.typeOf()` for block modifier', t => {
    ...
    t.true(t.context.stub.calledWith({ block: 'block', modName: 'mod', modVal: 'val' }));
});
...
test('should use naming.typeOf() for element modifier', t => {
    ...
    t.true(t.context.stub.calledWith({ block: 'block', elem: 'elem', modName: 'mod', modVal: 'val' }));
});
