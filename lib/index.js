...
function nested(item, add, walk, done) {
    var basename = item.basename,
        ...
        deep = filename.split(sep).length - item.level.split(sep).length;

    if ((deep === 1 && naming.isBlock(item.notation)) ||
        (deep === 2 && naming.isBlockMod(parent + basename)) ||
        (deep === 2 && naming.isElem(parent + basename)) ||
        (deep === 3 && naming.isElemMod(parent + basename))
    ) {
