...
var fs = require('fs'),
    ...
    deepExtend = require('lodash/merge'),
    path = require('path'),
    join = path.join,
    ...
    Object.keys(env)
        ...
        .forEach(function(key) {
            ...
            keypath.forEach(function _buildSubObj(_subkey, idx) {
                ...
                if (idx === keypath.length - 1) cursor[_subkey] = env[key];


                // Build sub-object if nothing already exists at the keypath
                if (typeof cursor[_subkey] === 'undefined') cursor[_subkey] = {};
...
function findAllConfigs(start, rel) {
    ...
    while (path.dirname(start) !== start) { // root
        ...
        fs.existsSync(file) && found.push(file);
...
module.exports = function(name, defaults, argv) {
    ...
    function addConfigFile (file) {
        ...
        if (fileConfig) {
            var parsedConfig = vm.runInNewContext('(' + fileConfig + ')');
    ...
    findAllConfigs(process.cwd(), '.' + name + 'rc').forEach(addConfigFile);

    if (env.config) addConfigFile(env.config);
    if (argv.config) addConfigFile(argv.config);

    return [defaults || {}].concat(configs, Object.keys(env).length ? env : []);
}
