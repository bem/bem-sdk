...
describe('create', () => {
  ...
  it('should create block from object', () => {
    ...
    expect(entityName instanceof BemEntityName).to.be.true;
  ...
  it('should create simple modifier of block from object', () => {
    ...
    expect(entityName.valueOf()).to.deep.equal({
      block: 'block',
  ...
  it('should use `mod.name` field instead of `modName`', () => {
    const entityName = BemEntityName.create({
      block: 'block',
      mod: { name: 'mod1' },
      modName: 'mod2',
    });
    expect(entityName.mod?.name).to.equal('mod1');
  });
  ...
  it('should use `mod.val` field instead of `modVal`', () => {
    ...
    expect(entityName.mod?.val).to.equal('v1');
  });
